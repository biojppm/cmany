...
# -----------------------------------------------------------------------------
class BuildItem:
    """A base class for build items. Provides easy conversion to str and representation."""
...
class Build:
    ...
    def create_preload_file(self):
        ...
        def _s(var, value, type):
            ...
            lines.append('_cmanydbg({})'.format(var))
            lines.append('')
        ...
        if len(lines) > 0:
            ...
            l2 = "# Generated by cmany on " + datetime.now().strftime("%Y/%m/%d %H:%m")
            lines.insert(0, l1)
            lines.insert(1, l2)
            lines.insert(2, "")
            lines.insert(3, 'message(STATUS "cmany:preload----------------------")')
            lines.insert(4, """function(_cmanydbg var)
message(STATUS "cmany: ${var}=${${var}}")
endfunction(_cmanydbg)
""")
            lines.append('message(STATUS "cmany:preload----------------------")')
            lines.append("")
            lines.append(l1)
            lines.append(l2)
